...
class AddOnsVC: UIViewController {
    ...
    let stepsStack = StepsStackView(step: 3)
    let flexButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.flexibleSpace, target: nil, action: nil)
    let addOnsModal = ModalAddOnsVC()
    ...
    func configure() {
        ...
        NSLayoutConstraint.activate([
            ...
            addOnsModal.view.topAnchor.constraint(equalTo: stepsStack.bottomAnchor, constant: 20),
            addOnsModal.view.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            addOnsModal.view.widthAnchor.constraint(equalToConstant: 350),
            addOnsModal.view.heightAnchor.constraint(equalToConstant: 550)
        ])
    }
    
    func addVCChild () {
        view.addSubview(addOnsModal.view)
        addChild(addOnsModal)
        addOnsModal.didMove(toParent: self)
    }
    ...
    @objc func goToNextController() {
        show(SummaryVC(addOns: addOnsModal.addOnsChoosed.addOnsChossed), sender: self)
    }
